...
class RegisterForm(FlaskForm):
    ...
    role = SelectField('Rôle', validators=[DataRequired()])
...
class BudgetForm(FlaskForm):
    mois = DateField('Mois (AAAA-MM-JJ)',
                     format='%Y-%m-%d',
                     validators=[DataRequired()])
...
class PlateformeForm(FlaskForm):
    ...
    nb_personnes_necessaires = IntegerField(
        'Personnel nécessaire',
        validators=[DataRequired(), NumberRange(min=1)])
    cout_journalier = DecimalField('Coût journalier',
                                   validators=[DataRequired()])
    intervalle_maintenance = IntegerField('Intervalle maintenance (jours)',
                                          validators=[DataRequired()])

    habilitations_requises = SelectMultipleField('Habilitations requises',
                                                 coerce=int,
                                                 validators=[Optional()])
    submit = SubmitField('Enregistrer')
...
class PersonnelForm(FlaskForm):
    ...
    habilitations = SelectMultipleField('Habilitations possédées',
                                        coerce=int,
                                        validators=[Optional()])
    submit = SubmitField('Enregistrer')
...
class CampagneForm(FlaskForm):
    ...
    plateforme = SelectField(  # On garde SelectField, mais on va gérer la validation différemment
        'Plateforme',
        validators=[
            DataRequired(message="Veuillez sélectionner une plateforme.")
        ],
        coerce=int)
    ...
    personnel_implique = SelectMultipleField(
        ...
        validators=[
            DataRequired(
                message="Veuillez sélectionner au moins un membre du personnel."
            )
        ])
    submit = SubmitField('Enregistrer')
...
class MaintenanceForm(FlaskForm):
    date_maintenance = DateField('Date de maintenance',
                                 format='%Y-%m-%d',
                                 validators=[DataRequired()])
    duree = IntegerField('Durée (jours)', validators=[DataRequired()])
    type_operation = StringField('Type d\'opération',
                                 validators=[DataRequired()])

    plateforme = SelectField('Plateforme concernée',
                             coerce=int,
                             validators=[DataRequired()])
    submit = SubmitField('Enregistrer')
